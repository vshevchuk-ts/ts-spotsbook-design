...
**Semantic groups (canonical):** surface (page/card/raised/**floating**/overlay/disabled) · fill (active/activeGradient/accent/positive/negative/warning/neutral/**contrast**/disabled + pre-composited hovers) · bg (active/accent/positive/negative/warning — 12% tints) · outline (default/strong/active/accent/positive/negative/warning) · text (default/secondary/contrast/forActiveBg/active/accent/positive/negative/warning/onFill) · icon (mirrors text) · betStatus (win/loose/halfWin/halfLoose/refund/cashout/pending) · label (live/freebet/postponed/reviewing/score/inactiveCounter/activeCounter) · darken (2/4/6/8) · lighten (2/4) · shadow (2/4/6/8) · additional (favorites/preloader).
- **`surface.floating`** (= surface-6) — a surface that floats above the card/raised stack, lifted by a shadow not a border (Toast). The sanctioned surface-6-as-background role.
- **`fill.contrast`** (= base.contrast, theme-adaptive white) — a small solid shape that must pop off a coloured/active fill (Counter active pill, Switch lit thumb).
...
Component progress:
- ✅ **Bet selection card** (`tokens/components/bet-card.tokens.json` + `build-bet-card-doc.mjs` → `docs/bet-card.html`, nav under a new **Betslip** category in both DS Components and index.html). Two densities off one anatomy: `--compact` (Combo/System, inline odds) and `--amount` (Single, per-selection Bet-amount well). Spec locked with the user: radius.md (12px), 8px padding, 8/4 vertical rhythm; outcome 14px bold, odds 16px bold (weight.bold), tabular-nums; event = link-sm underline (text.secondary, truncates); remove × = 24px, icon.secondary, lighten.2 hover; settlement info = icon.warning (only on special-rules markets); the Single stake field **is the Input component** (lg / 48px + currency prefix), resolved from input.tokens.json — regular 16px value, surface.page fill, outline.strong border, outline.active on focus; NOT restyled (only its 160px width + bottom-alignment are bet-card layout). Third density **`--betbuilder`**: header + an inset **surface-2** legs card (radius.md, 16/8 row padding, outline.default dividers) with each leg an outcome-over-market row (inverted order: outcome above, market below; reuses outcome/market roles) + a 20px ghost remove ×, then a footer (Betbuilder Odd label + odds value + the Input stake field, shown empty). Header now carries optional **LIVE / BB / FB badges** — the real **Badge** component at `sm` (16px), `named` variants (live/betbuilder = active tint, freebet = accent tint), resolved from badge.tokens.json and emitted as real `.badge` classes (composed, not redrawn); any combination or none. Event shrinks/truncates before them, × pushed to the far right (margin-left:auto). Info icon before the market is likewise optional per card. **Deferred to later passes** (called out in the page legend): odds *movement* → Odds component; LIVE/FB/BB → Badge variants; suspended (lock) + per-card error strip → card states; the amount field's ticket-notch silhouette → Input bet-amount variant.
- ✅ **Alert** (`tokens/components/alert.tokens.json` + `build-alert-doc.mjs` → `docs/alert.html`). Inline in-flow status banner, one style: 12% tint bg.* + colored outline.* border + status icon, white message. Four roles (warning/negative/positive/info — info = accent), each optionally with a close × — no title (an Alert is one message line in a status colour; the two axes are just role × dismissible). Negative reuses the warning triangle glyph (no error-circle in the UI set yet), differentiated by colour. (The solid full-bleed global bar was moved OUT to Toast `--global` per review — it's a docked notification, not an inline banner.)
- ✅ **Toast** (`tokens/components/toast.tokens.json` + `build-toast-doc.mjs` → `docs/toast.html`). Reworked per user review. Floating **surface-6** pill (a sanctioned surface-6-as-background exception) lifted by **elevation.sm** shadow (no border). Heading = **heading-base (semibold)**, not a bespoke bold title. Content is vertically centred (`align-items:center`; icon top-aligns only on two-line). **Two axes**: content (one line = heading only · two lines = + secondary — most messages are one line) × trailing control (none · close × · Undo · auto-dismiss timer; timer combines with a button). **Undo = the real Button primary/base (40px)** — size/label/colours resolved from button.tokens.json, not re-declared. Roles success/warning/negative/neutral colour the icon + timer tint, each with its own status glyph (success→check, warning/negative→triangle, negative reuses the triangle in red — no error-circle glyph yet). **Quick-Bet** is a separate pair of toasts (not the status roles): activated (filled gold bolt) / deactivated (grey crossed-out bolt). **`--global`**: the solid full-bleed app-wide bar (moved from Alert), warning only this pass (solid negative/positive await text.forLabelBg). Stacks vertically.
...
- `toast.bg` `{color.base.surface-6}` → new **`surface.floating`**.
- `counter.onPrimary.active.bg` `{color.white}` → new **`fill.contrast`**.
- `switch` thumb: off `{color.base.secondary}`→`{icon.secondary}`, on `{color.white}`→`{icon.onFill}`, disabled `{color.base.contrast}`→`{icon.onFill}` (thumb = an indicator on a fill).
- Re-audit is clean: `grep -rE '\{color\.(base|status|sub|white|black)' tokens/components/` → 0. Resolved hex are byte-identical to before (no visual change).

**Builders de-hardcoded:** switch/counter/toast/alert/bet-card now resolve these colours from their token nodes via a `cvOf(node)` helper (reads `node.$value`) instead of hardcoded `cv("role")`; counter's `colorPaths` is now **auto-collected** from its surface token subtree. Root-caused the recurring "changed the token, nothing happened" bug (builders were hardcoding roles — see CLAUDE.md hard rules).

**Component reuse fixed:** the close × in bet-card/alert/toast is now the **Button ghost** treatment (transparent → `fill.neutralHover`, `icon.secondary`→`text.default`), not a bespoke `lighten.2` overlay. Toast Undo = **Button primary/base**. Bet-card stake field = the **Input** component (lg + prefix). 

**Mistakes logged (now CLAUDE.md hard rules):** (1) hand-drawing a field/button instead of embedding the real component — caught by a font drift (bespoke semibold stake value vs Input's regular 16px); (2) builders hardcoding colour roles so token edits silently no-op; (3) a component reaching into a primitive when no semantic role existed instead of adding one / asking.
...
9. ✅ **Sportsbook button variants → real tokens.** The three product-specific button variants (`twoRow`, `roundIcon` outline/filled-neutral, `betslip` pill) were colour/size-hardcoded inside `build-button-doc.mjs`; they're now proper subtrees under `component.button` in `button.tokens.json`, and the builder resolves their CSS from those tokens (semantic colour refs via a small `cvT`/`dimT`/`typoCss` reader, plus `collectColorRefs` that auto-unions their colour paths into the `:root` var list — no hand-kept `colorPaths` for them). Effect: the semantic legend's "Used in" index now credits **Button** for the roles these variants introduced (`surface.card`, `surface.raised`, `outline.default`, `outline.strong`, `lighten.2`, `darken.2`, and `text.secondary`'s twoRow use) with per-state contexts. Generated CSS is byte-for-byte identical to the old hardcoded output.

10. ✅ **Assets migration — real product art replaces Material icons.** The 142-icon Material set (`assets/icons/material-filled/`) was **deleted** and replaced by four imported asset folders: `assets/icons/ui/` (62 Turbo Sportsbook UI glyphs), `assets/icons/sports/` (75 sport/e-sport discipline glyphs), `assets/images/teams/` (40 team logos, PNG), `assets/images/championships/` (18 league logos, PNG). On import (one-off script), SVGs had their monotone brand grey `#B8B8B8` → `fill="currentColor"` (multi-tone `red-card`/`yellow-card`/`globe`-gradient kept), Figma clip-path frames stripped, and internal ids namespaced by slug to avoid collisions when many are inlined. **Nav** grew an **Assets** category (`nav.mjs` + the hand-authored `index.html`): UI icons (repurposed `icons.html`/`build-icons-doc.mjs` → reads `ui/`), Discipline icons (`disciplines.html`), Teams (`teams.html`), Championships (`championships.html`). The two image pages group by sport parsed from the `"<Sport> - <Name>.png"` filename (the one `Dota2` typo normalized to `Dota 2`); shared shell in `tools/lib/asset-page.mjs`. **Components repointed** to `ui/` equivalents in all 13 builders (search/close exact; add→plus, arrow_forward→arrow-right, chevron_left/right→arrow-left/right, expand_more→arrow-down, mail→my-bets, flag→favorites, tune→filters, edit→settings, content_copy→copy, listbox check→check-circle). Checkbox's check/indeterminate mark is now **inline geometry** (the UI set ships only composite check-in-box glyphs, no bare tick). All 31 doc pages (30 generated + hand-authored `index.html`) build clean.
