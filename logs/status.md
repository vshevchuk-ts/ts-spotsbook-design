...
- ✅ **Alert** (`tokens/components/alert.tokens.json` + `build-alert-doc.mjs` → `docs/alert.html`). Inline status banner. Two styles: `outline` (12% tint bg.* + colored outline.* border + status icon, white message) for all four roles (warning/negative/positive/info — info = accent); `--solid` (solid fill + dark text) for the app-wide global bar, **warning only this pass** (solid negative/positive/info await the deferred text.forLabelBg). Optional bold title + close ×. Negative reuses the warning triangle glyph (no error-circle in the UI set yet), differentiated by colour.
- ✅ **Toast** (`tokens/components/toast.tokens.json` + `build-toast-doc.mjs` → `docs/toast.html`). Transient top-docked confirmation on surface.raised. Roles success/warning/neutral set the leading-icon colour + progress tint (glyph passed per toast: check-circle / quick-bet / quick-bet-exit). Optional Undo action (reuses fill.active + text.forActiveBg at 36px), close ×, and a 3px auto-dismiss progress meter (role-tinted over a lighten.2 track, animates width→0, respects prefers-reduced-motion). Stacks vertically.
