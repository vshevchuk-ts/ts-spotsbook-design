...
- ✅ **Alert** (`tokens/components/alert.tokens.json` + `build-alert-doc.mjs` → `docs/alert.html`). Inline in-flow status banner, one style: 12% tint bg.* + colored outline.* border + status icon, white message. Four roles (warning/negative/positive/info — info = accent). Optional bold title + close ×. Negative reuses the warning triangle glyph (no error-circle in the UI set yet), differentiated by colour. (The solid full-bleed global bar was moved OUT to Toast `--global` per review — it's a docked notification, not an inline banner.)
