...
- **Every inlined SVG icon MUST be clamped to its container** with a `.wrapper svg { display: block; width: 100%; height: 100%; }` rule. The imported `assets/icons/**` SVGs carry their own `width="20"/height="20"` attributes, which otherwise win over the container's `width/height`, so the icon renders at its native size (oversized) and inflates the row. Setting the wrapper size alone is NOT enough. This bit twice on the bet card (sport icon, then the info icon — "опять 25"). When you size an icon wrapper, add the `svg { width:100%; height:100% }` rule in the same edit.
...
- **Compose out of existing components — never redraw one.** When a component needs a button, field, counter, etc., embed the real component: reuse its tokens AND its treatment (a close × is the **Button ghost** icon button → `icon.secondary`→`text.default`, transparent→`fill.neutralHover`; a primary action is **Button primary**; a stake field is the **Input** component — lg + prefix, resolved from `input.tokens.json`). If the sub-element's font/size/colour drifts from the real component (e.g. a stake value rendering semibold when Input is regular 16px), that's the tell you hand-drew it — stop and embed the component. Cost this session: a bespoke bet-amount field and three hand-rolled close buttons had to be re-based onto Input / Button ghost.
- **Builders never hardcode a colour role or a primitive.** Every colour in generated CSS must be resolved from the component's own token node — `cvOf(node)` reading `node.$value` — so a token repoint flows through with zero build edits. Do NOT write `cv("icon.secondary")` when the value should come from `switchTok.state.default.thumb`; the token edit alone must be enough. `colorPaths` (the `:root` var-emission list) holds **semantic roles only, never primitives** — prefer auto-collecting it from the token subtree (see `build-counter-doc.mjs`) so it can't drift or leave a var undefined. This is the root of the "token changed but nothing happened" class of bug.
- **Components alias semantic roles, never primitives, for colour.** No `{color.base.*}` / `{color.status.*}` / `{color.sub.*}` / `{color.white}` / `{color.black}` in a component token file. If no semantic role fits what you need, **add one to `tokens/semantic/color.tokens.json`** (e.g. `surface.floating` = surface-6 for floating notifications; `fill.contrast` = a white/contrast pill fill) — and if the right name/approach is unclear, **ask rather than reaching into a primitive**. Audit with `grep -rE '\{color\.(base|status|sub|white|black)' tokens/components/` — expected result is zero.
